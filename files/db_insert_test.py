...
def insert_check(directory):
            ...
            if file.endswith(".json"):
                ...
                try:  # try to load the JSON data
                    with open(file_path, "r") as f:
                        data = json.load(f)
                    if data:   # check if data exists in the file
                        subdirectory_data.append(data)
                        total_files += 1  # increment the total files counter 
                    else:
                        print(f"File {file} is empty. No data is inserted.")
    ...
    # Check if any valid data-containing file was found
    if total_files == 0:
        print("No data found in any file in any subdirectories.")
        return
    ...
    if answer.lower() == "yes":
        ...
        # check if any suitable data has been filtered
        if not filtered_data:
            print("No suitable data found for the specified date.")
            return

        for item in filtered_data:
            item['Timestamp'] = convert_timestamp_to_datetime(item['Timestamp'])

        n_inserts = 0  # counter of inserted rows
        try:
            cursor = cnx.cursor()
            ...
            if n_inserts > 0:  # only print the success message when we actually inserted something
