...
session_data <- read.csv("data/training.csv")
...
temperature_colors <- colorRampPalette(c("#5B9BD5", "#003366"))(length(unique(session_data$ShortMAC)))  # Shades of blue
moisture_colors <- colorRampPalette(c("#70AD47", "#2F6A21"))(length(unique(session_data$ShortMAC)))     # Shades of green
light_colors <- colorRampPalette(c("#FFC000", "#FF7F00"))(length(unique(session_data$ShortMAC)))        # Shades of yellow-orange
conductivity_colors <- colorRampPalette(c("#ED7D31", "#9C4A2F"))(length(unique(session_data$ShortMAC))) # Shades of orange

# Create a color map based on ShortMAC for each variable
color_map_temperature <- setNames(temperature_colors, unique(session_data$ShortMAC))
color_map_moisture <- setNames(moisture_colors, unique(session_data$ShortMAC))
color_map_light <- setNames(light_colors, unique(session_data$ShortMAC))
color_map_conductivity <- setNames(conductivity_colors, unique(session_data$ShortMAC))

# Plot Temperature over Time with different shades for each MAC address
plot(session_data$Timestamp, session_data$Temperature,
     col = color_map_temperature[session_data$ShortMAC],  # Color by MAC address
     xlab = "Time",
     ylab = "Temperature",
...
plot(session_data$Timestamp, session_data$Moisture,
     col = color_map_moisture[session_data$ShortMAC],  # Color by MAC address
     xlab = "Time",
     ylab = "Moisture",
...
plot(session_data$Timestamp, session_data$Light,
     col = color_map_light[session_data$ShortMAC],  # Color by MAC address
     xlab = "Time",
     ylab = "Light",
     main = "Weekly Light",
     pch = 16)
...
plot(session_data$Timestamp, session_data$Conductivity,
     col = color_map_conductivity[session_data$ShortMAC],  # Color by MAC address
     xlab = "Time",
     ylab = "Conductivity",
